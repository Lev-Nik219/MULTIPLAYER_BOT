...
def get_queue_size(game_type: str) -> int:
    return len(matchmaking_queues.get(game_type, []))

def add_to_queue(telegram_id: int, game_type: str, bet: int, db: Session) -> Optional[int]:
    """Добавляет игрока в очередь поиска. Возвращает opponent_id если найден"""
...
def remove_from_queue(telegram_id: int, game_type: str):
    queue = matchmaking_queues.get(game_type, [])
    matchmaking_queues[game_type] = [q for q in queue if q[0] != telegram_id]
...
class DuelClickerGame:
    ...
    def finish(self, db: Session) -> dict:
        ...
        if winner:
            result = finish_game(db, self.game_id, winner, self.scores[self.player1], self.scores[self.player2])
            return result
        else:
